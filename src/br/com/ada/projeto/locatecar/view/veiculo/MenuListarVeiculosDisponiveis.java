...
public class MenuListarVeiculosDisponiveis extends Submenu {
    private final GerenciadorDeVeiculo gerenciadorDeVeiculo;

    public MenuListarVeiculosDisponiveis(GerenciadorDeVeiculo gerenciadorDeVeiculo) {
        super("Listar todos os " + gerenciadorDeVeiculo .getDescricaoPluralDaEntidade() + " disponíveis");
        this.gerenciadorDeVeiculo = gerenciadorDeVeiculo;
    }

    @Override
    public void acao() {
        MenuDePaginacao<Veiculo> menuDePaginacao = new MenuDePaginacao<>(gerenciadorDeVeiculo.getDescricaoPluralDaEntidade(), gerenciadorDeVeiculo.listarDisponiveisComPaginacao(ConstanteDePaginacao.MAXIMO_DE_ELEMENTOS_POR_PAGINA));
